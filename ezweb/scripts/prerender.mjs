 ...
 * dist/<path>.html (dist/index.html for "/", dist/404.html for "/404") with the
...
const outFileFor = (p) => {
  ...
  // Flat "<path>.html" files, not "<path>/index.html". Netlify, Vercel (cleanUrls) and
  // Cloudflare Pages all serve dist/about.html at /about with no redirect, which matches our
  // slash-free canonicals. A directory index would make Netlify 301 /about -> /about/.
  const clean = p.replace(/^\/+|\/+$/g, '')
  return path.join(distDir, ...clean.split('/').slice(0, -1), `${clean.split('/').pop()}.html`)
}
...
if (fs.existsSync(redirectsOut)) {
  const slashRules = pages
    .filter((pg) => pg.path !== '/' && !pg.noindex)
    .map((pg) => `${pg.path}/ ${pg.path} 301`)
  fs.appendFileSync(redirectsOut, `\n# Trailing-slash variants of every prerendered page (generated by scripts/prerender.mjs)\n${slashRules.join('\n')}\n`)
}
